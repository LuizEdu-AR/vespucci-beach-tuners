...
export default function HierarchyPage(){
  ...
  const personRow=(person,pendingUser=false)=><div className="person-row" key={person.id}><div className="person-main"><div className="avatar small">{person.photoURL?<img src={person.photoURL} alt=""/>:person.name?.charAt(0)}</div><div><strong>{person.name}</strong><span>ID {person.rpId}{person.contactRP?` · ${pendingUser?'Contato ':''}${person.contactRP}`:''}</span></div></div>{editing===person.id?<div className="role-edit"><select value={role} onChange={e=>setRole(e.target.value)}>{assignableRoles.map(r=><option key={r}>{r}</option>)}</select><button className="icon-button success" onClick={()=>save(person)} title="Salvar cargo"><Save size={17}/></button><button className="icon-button" onClick={()=>setEditing(null)}><X size={17}/></button></div>:pendingUser?<button className="button small" onClick={()=>start(person)}><UserPlus size={16}/> Atribuir cargo</button>:<div className="person-actions"><RoleBadge role={person.role}/>{person.id!==user.uid&&canManageRole(person.role)&&<><button className="icon-button" onClick={()=>start(person)} title="Alterar cargo"><Pencil size={16}/></button><button className="icon-button danger" onClick={()=>fire(person)} title="Demitir funcionário"><UserMinus size={16}/></button></>}</div>}</div>

  return <>
    <div className="page-heading"><div><span className="eyebrow">EQUIPE</span><h1>Hierarquia</h1><p>Dono, Diretor e Gerente podem gerenciar apenas cargos abaixo do próprio nível hierárquico.</p></div></div>
    ...
    <div className="hierarchy-grid">{ROLE_GROUPS.map(group=>{const people=staff.filter(p=>p.role===group);return <section className="role-section" key={group}><div className="role-section-title"><div><Crown size={17}/><strong>{group==='Dono'?'PROPRIETÁRIOS':group.toUpperCase()}</strong></div><span>{people.length}</span></div>{people.length===0?<div className="vacant">• (Vago)</div>:people.map(person=>personRow(person,false))}</section>})}</div>
  </>
}
