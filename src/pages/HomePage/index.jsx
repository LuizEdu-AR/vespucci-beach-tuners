...
export default function HomePage(){
 ...
 return <><div className="page-heading"><div><span className="eyebrow">PAINEL</span><h1>Olá, {user.name.split(' ')[0]}.</h1><p>Gerencie os serviços da Vespucci Beach Tuners em um só lugar.</p></div></div><div className="hero-card"><div><span className="hero-kicker">OFICINA ONLINE</span><h2>Controle rápido, histórico organizado e preços padronizados.</h2><p>Informações organizadas e atualizadas para uma experiência mais prática.</p></div><div className="hero-id"><span>SEU ID</span><strong>#{user.id}</strong><small>{user.role}</small></div></div><div className="dashboard-grid">{cards.map(([title,sub,Icon,to])=><Link className="dash-card" to={to} key={title}><div className="dash-icon"><Icon/></div><div><strong>{title}</strong><span>{sub}</span></div><span className="card-arrow">→</span></Link>)}</div></>
}
