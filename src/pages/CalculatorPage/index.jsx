...
export default function CalculatorPage(){
 ...
 <div className="content-grid calculator-layout"><div className="stack">
   ...
   <div className="card"><h3>Blindagem</h3><div className="option-row"><div><strong>Nível de blindagem</strong><span>Full: $ {money(prices.fullBlindagem)}</span></div><select value={sel.blindagem} onChange={e=>setSel({...sel,blindagem:e.target.value})}><option value="">Não aplicar</option>{(prices.blindagem||[]).map((v,i)=><option key={`blind-${i}`} value={String(i+1)}>Nível {i+1} — $ {money(v)}</option>)}<option value="full">Full Blindagem — $ {money(prices.fullBlindagem)}</option></select></div></div>
